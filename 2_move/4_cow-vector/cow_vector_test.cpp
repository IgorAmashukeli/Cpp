...
// if you wrote move constructor and move assignment operator and not deleted them
/**TEST_CASE("Correct move constructor") {
    COWVector v1;
    v1.PushBack("foo");
    v1.PushBack("bar");
    v1.PushBack("baz");
    auto str_ptr = &v1.Back();
    COWVector v2 = std::move(v1);
    REQUIRE(v2.Size() == 3);
    REQUIRE(v2.Get(0) == "foo");
    REQUIRE(v2.Get(1) == "bar");
    REQUIRE(v2.Get(2) == "baz");
    REQUIRE(str_ptr == &v2.Back());
}**/

/**TEST_CASE("Correct move assignment operator") {
    COWVector v1;
    COWVector v2;
    v1.PushBack("foo");
    v1.PushBack("bar");
